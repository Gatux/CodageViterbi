...
\section{Décodage des codes convolutifs : algorithme de Viterbi}
	...
	\subsection{Exemple d'application de l'algorithme de Viterbi}
		...
		\end{figure}
		\\
		On parcourt le treillis en partant de la fin jusqu'à revenir à l'état initial.\\
		Dans cet exemple, on obtient la séquence de bits $00111$.\\
		Il ne reste plus qu'à inverser l'ordre de la séquence et enlever les $M=2$ derniers bits correspondant à la remise à zéro des registres à décalage pour obtenir le message envoyé, qui est ici $\textbf{m} = [111]$.
